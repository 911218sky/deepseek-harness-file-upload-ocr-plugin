window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function FileAttachmentRail({ sessionId, useInput, files, remove }) {
			...
			const snapshot = (0, react.useSyncExternalStore)((listener) => sessionId === void 0 ? () => void 0 : files.subscribe(sessionId, listener), () => sessionId === void 0 ? EMPTY_FILES$1 : files.get(sessionId));
			const activeRefs = (0, react.useMemo)(() => new Set(occurrences.filter((item) => item.source === FILE_SOURCE).map((item) => item.ref)), [occurrences]);
			const active = sessionId === void 0 ? EMPTY_FILES$1 : snapshot.filter((file) => activeRefs.has(file.ref));
			...
			if (sessionId === void 0 || active.length === 0) return null;
		...
		*/
		function OcrComposerAttachments({ NativeAttachments, files, remove, sessionId, useInput, ...nativeProps }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [NativeAttachments !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(NativeAttachments, {
				...nativeProps,
				sessionId,
				useInput
		...
		*/
		...
		function apply(ctx) {
			...
			ctx.slots.inject("conversation.input.attachments", () => {
				const NativeAttachments = ctx.slots.entries("conversation.input.attachments").find((entry) => (entry.options.priority ?? 0) === 0)?.component;
