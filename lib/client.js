window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function FileAttachButton({ attach, attachImage, beginExtract, failExtract, clearPending, hasPending }) {
		...
		function useResolvedSessionId(ocrSessionId, files) {
		...
		function AttachmentCards({ ocrSessionId, useInput, files, remove, layout }) {
			const session = useResolvedSessionId(ocrSessionId, files);
			const occurrences = useInput((state) => state?.occurrences ?? []);
			const draft = useInput((state) => state?.draft ?? "");
			const phase = useInput((state) => state?.phase);
			const generation = (0, react.useSyncExternalStore)((listener) => files.subscribeGlobal(listener), () => files.getGeneration());
			const [composerRailLive, setComposerRailLive] = (0, react.useState)(false);
			(0, react.useEffect)(() => {
			...
			const ready = session === void 0 ? EMPTY_FILES : files.get(session);
			const pending = session === void 0 ? EMPTY_PENDING$1 : files.getPending(session);
			const activeRefs = (0, react.useMemo)(() => new Set(occurrences.filter((item) => item.source === FILE_SOURCE).map((item) => item.ref)), [occurrences]);
			const active = session === void 0 ? EMPTY_FILES : ready.filter((file) => activeRefs.has(file.ref));
			...
			(0, react.useEffect)(() => {
				...
				const delay = activeRefs.size === 0 && draftEmpty ? 250 : 1e3;
			...
			if (session === void 0 || active.length === 0 && pending.length === 0) return null;
			...
			if (layout === "dock") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: import_FileAttachments_module_css.default.dock,
				"data-ocr-layout": "dock",
				"aria-label": "已添加的文件 / Added files",
				children: body
			});
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: import_FileAttachments_module_css.default.composerRail,
				"data-ocr-layout": "composer",
				"aria-label": "已添加的文件 / Added files",
				children: body
			});
		}
		/** OCR/file cards styled like native FileCard, rendered inside the composer. */
		function FileAttachmentRail(props) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AttachmentCards, {
				...props,
				layout: "composer"
			});
		}
		/** Same cards above the composer — reliable fallback when attachments shadow fails. */
		function FileAttachmentDock(props) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AttachmentCards, {
