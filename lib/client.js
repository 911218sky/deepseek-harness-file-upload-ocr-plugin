window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function AttachmentCards({ ocrSessionId, useInput, files, remove, layout }) {
			...
			const active = session === void 0 ? EMPTY_FILES : activeRefs.size > 0 ? ready.filter((file) => activeRefs.has(file.ref)) : ready;
			const refKey = [...activeRefs].join("\0");
			(0, react.useEffect)(() => {
				if (session === void 0 || phase === "submitting") return;
